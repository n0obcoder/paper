...
class Vocabulary(object):
    def __init__(self, vocab_file, emb_file='', dim_emb=0):
        ...
        # random initialization of the embeddings
        self.embedding = np.random.random_sample(
            (self.size, self.dim_emb)) - 0.5

        if emb_file: 
            print('\nLoading word vectors from', emb_file)
        ...
        # print(self.embedding.shape) # [9357, 100]

        # applying L2 norm to all the embeddings
...
def build_vocab(data, path, min_occur=5):
    ...
    # list of all the occurances of words in data 

    cnt = Counter(words)
    # it is like a dictionary of all unique words with their occurances as their values
